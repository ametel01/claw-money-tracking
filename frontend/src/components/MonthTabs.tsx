...
export function MonthTabs({ months, activeMonth, onMonthChange }: MonthTabsProps) {
  ...
  return (
    <div className="flex gap-1.5 overflow-x-auto pb-0.5 snap-x snap-mandatory [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {months.map((month) => {
        ...
        return (
          <button
            ...
            className={cn(
              'snap-start whitespace-nowrap rounded-sm border px-3 py-1.5 text-[0.68rem] font-semibold tracking-wide transition-colors',
              isActive
